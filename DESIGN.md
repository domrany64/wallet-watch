...
- Inter-bank transfers: `real time payment`, `rtp rec`, `rtp-`, `funds tran`, `internet trf`, `onpoint ccu`, `upgrade crb`, `upgrade.*transfer`, `^transfer$`, `^transfer to`, `^transfer from`, `external transfer`
...
- Filter bar: description search, by card, spender, category, type (expenses/income/refunds), sort (date/amount asc/desc) — all persistent across month navigation
